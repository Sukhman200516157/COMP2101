...
[ "$verbose" = "yes" ] && echo "Next we will use $1 on the command line"

#Now we make use of case command in the command line
case "$1" in

#For -v on the command line, we will set variable
...
#Using case command to find if there is any string in the command line
    ...
    [0-9])
        if [ "$verbose" = "yes" ]; then
            ...
            echo $(netid.sh)
            echo "Only interface information"
            exit
        fi
        ;;
...
#Using LIST word to get a list of ip interface
    LIST )
        if [ "$verbose" = "yes" ]; then
            echo "Command line  processed"
            exit
#using else command 
        else
            for e in $(ifconfig | awk '/: /{print $2}'); do
            echo "$e"
        done
        fi
        ;;
